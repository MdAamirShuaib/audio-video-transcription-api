...
@app.post('/{use_id}/')
async def user(request: Request,  use_id , username: str = Form(...), password: str = Form(...)):
  select_box_html,dir_name = authenticate_user(username,password)
...
async def transcribe(*,project_name: str = Form(...),files: List[UploadFile] = File(...),
                              request: Request,dir_name,use_id,username):
  html,exception_html,username,use_id=start_transcribe(project_name,files,dir_name,use_id,username)
...
async def get_records(request: Request):
  ...
  for re in table_rows:
    table+="<tr><td>"+re[1].strftime("%d-%m-%Y %I:%M %p")+"</td><td>"+re[2]+"</td><td>"+re[3]+"</td><td>"+re[4]+"</td>"
    table+="<td><a href='"+re[5]+"' class='dwn-link'>Download</a></td>"
    table+="<td><a href='"+re[6]+"' class='dwn-link'>Download</a></td></tr>"
