...
class Connect:
    def initiate_db():
        ...
        if "transcriptions" not in databases:
            ...
            start_cursor.execute("CREATE TABLE `transcriptions`.`Projects` (`id` INT NOT NULL AUTO_INCREMENT,`project_name` VARCHAR(200) NULL,`username` VARCHAR(50) NULL,PRIMARY KEY (`id`),UNIQUE INDEX `project_name_UNIQUE` (`project_name` ASC) VISIBLE);")
            db.commit()
...
def json_data_extraction(result,fname):
    ...
    topics['label_2'] = topics['labels'].apply(lambda x: x[1]['label'] if len(x)>1 else 'none')
    topics['label_3'] = topics['labels'].apply(lambda x: x[2]['label'] if len(x)>2 else 'none')
    ...
    group = ['file_name','speaker', 'summary', 'headline','gist', 'label_1', 'label_2', 'label_3', 'sequence']
    df = pd.DataFrame(audindex.groupby(group).agg(utter=('text', ' '.join), start_time=('start', 'min'),end_time=('end', 'max')))
    ...
    df_doc = pd.DataFrame(audindex.groupby(group_doc).agg(utter=('text', ' '.join), start_time=('start', 'min')))
...
def start_transcribe(project_name,files,dir_name,use_id,username):
    ...
    try:
        for file in files:
            ...
            pre_cursor.execute("select username,project_name,fname,doc_download_link,csv_download_link,transcription_time from Records where project_name ='"+project_name+"' and fname ='"+fname+"';")
            table_rows = pre_cursor.fetchall()
            for re in table_rows:
                html+="<tr><td>"+re[0]+"</td><td>"+re[1]+"</td><td>"+re[2]+"</td>"
                html+="<td><a href='"+re[3]+"' class='dwn-link'>Download</a></td>"
                html+="<td><a href='"+re[4]+"' class='dwn-link'>Download</a></td>"
                html+="<td>"+re[5]+"</td><td>existing</td></tr>"
            file_list= []
            for row in table_rows:
                file_list.append(row[2])
                exception_html+=row[2]+":&nbsp;&nbsp;  already exists in the database<br>"
            if fname not in file_list:
                ...
                while result.get("status") != 'completed':
                    ...
                    if result.get("status") == 'error':
                        html+="<tr><td>"+username+"</td><td>"+project_name+"</td><td>"+fname+"</td><td>-</td><td>-</td><td>-</td><td>error</td></tr>"
                        exception_html+=fname+": please check the file and try again <br>"
                        error_log = open("logs/error_log.txt","a+")
                        error_str = datetime.now().strftime("%m-%d-%Y, %I:%M %p")+":     "+username+":     ["+fname+"]       : AssemblyAI Processing Error"
                        error_log.write(error_str+"\n")
                        error_log.close()
                        return HTMLResponse(content=html_completed(html,exception_html,username,use_id), status_code=200)      
                transcription_time = str(round((time.time()-transcription_start_time)/60,2))+" mins"
                print("transcription completed")
                df, doc = json_data_extraction(result,fname)

                print('Saving the Transcribed Files to S3: [ {} ]'.format(fname))
                df.to_csv("documents/"+dir_name+"/"+project_name+"/"+fname+".csv",index=False)
                doc.save("documents/"+dir_name+"/"+project_name+"/"+fname+".docx")
                ...
                os.remove("documents/"+dir_name+"/"+project_name+"/"+fname+".docx")
                os.remove("documents/"+dir_name+"/"+project_name+"/"+fname+".csv")
                ...
                doc_download_link="http://ec2-54-164-248-248.compute-1.amazonaws.com/download/"+project_name+"/"+fname+".docx"
                csv_download_link="http://ec2-54-164-248-248.compute-1.amazonaws.com/download/"+project_name+"/"+fname+".csv"
                record = [( datetime.now(),username, project_name, fname, doc_download_link, csv_download_link, transcription_time),]
                record_cursor.executemany(query, record)
                mydb.commit()
                html+="<tr><td>"+username+"</td><td>"+project_name+"</td><td>"+fname+"</td>"
                html+="<td><a href='"+doc_download_link+"' class='dwn-link'>Download</a></td>"
                html+="<td><a href='"+csv_download_link+"' class='dwn-link'>Download</a></td>"
                html+="<td>"+transcription_time+"</td><td>success</td></tr>"
    except Exception as e:
        error_log = open("logs/error_log.txt","a+")
        error_str=datetime.now().strftime("%m-%d-%Y, %I:%M %p")+": "+username+": ["+fname+"] "
